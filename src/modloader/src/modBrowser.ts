	...
	if (modsOnGithub) {
...

		{
			...
			modsOnGithub.forEach((mod: ModsBrowserMod) => {
				if (
					(mod.name.toLowerCase().includes(LoaderData.getData('ModBrowserSearched', '', false).toLowerCase()) || mod.author.toLowerCase().includes(LoaderData.getData('ModBrowserSearched', '', false).toLowerCase())) &&
					ImGui.Selectable(`${mod.name} | ${mod.author}##ModUrlSelectable${mod.author}`, selectedMod === mod)
				) {
